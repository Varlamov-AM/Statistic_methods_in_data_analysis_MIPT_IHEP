                     ...
                     const char* dataName = "data" )
{
  ...
  bayesianCalc.SetConfidenceLevel(0.683); // 68% interval   // set the type of interval (not really needed for central which is the default)   
  bayesianCalc.SetLeftSideTailFraction(0.5); 
  // for central interval   
  //bayesianCalc.SetLeftSideTailFraction(0.); 
  // for upper limit   //bayesianCalc.SetShortestInterval(); 
  ...
  TString integrationType = "";   // this is needed if using TOYMC   
  ...
  bayesianCalc.SetIntegrationType(integrationType);    // compute interval by scanning the posterior function   
